...
void InitSpi(void)
{
   // Initialize SPI-A/B
    // Initialize SPI-A
    InitSpiaGpio();     //旋变的SPI接口
    Spia_init();        //

    spi_fifo_init();    // AD7606采样初始化
    InitSpibGpio();
    Spib_init();
   //tbd...
...
//  GpioCtrlRegs.GPAMUX1.bit.GPIO5 = 2;  // Configure GPIO5 as SPISIMOA
    ...
    GpioCtrlRegs.GPAMUX1.bit.GPIO11 = 0;    //  Configure GPIO19 as GPIO (模拟片选)
    GpioCtrlRegs.GPADIR.bit.GPIO11 = 1;     //配置为输出
...
void Spia_init(void)
{
    ...
    SpiaRegs.SPICCR.bit.CLKPOLARITY = 0;        // CLK平时为低电平
    SpiaRegs.SPICTL.bit.CLK_PHASE = 0;          // 下降沿发数据，上升沿收数据
...
void Spib_init()
{
    ...
    SpibRegs.SPICCR.bit.CLKPOLARITY = 1;        // 上升沿发数据，下降沿收数据
    SpibRegs.SPICTL.bit.CLK_PHASE = 1;          // 无相位延迟
